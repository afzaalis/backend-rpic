...
// Route untuk Update status booking menjadi 'completed'
router.put('/:bookingId/completed', async (req, res) => {
  const { bookingId } = req.params;
  ...
  try {
    ...
    // Pastikan status booking saat ini bukan 'completed'
    if (booking.status === 'completed') {
      return res.status(400).json({ error: 'Booking is already completed' });
    }

    // Update status booking menjadi 'completed'
    const updateResult = await updateBookingStatus(bookingId, 'completed');
    ...
    console.error('Error updating booking status to completed:', err);
    res.status(500).json({ error: 'Failed to update booking status' });
  }
});
